...
namespace CreatePaymentForOnlineApplication
{
    ...
    {      
        ...
        public static EntityReference SearchForRef(IOrganizationService service, string entityName, string searchField, object searchValue)
        {
            EntityReference result = null;
            Guid searchResult = SearchFor(service, entityName, searchField, searchValue);
            if (searchResult != Guid.Empty)
                result = new EntityReference(entityName, searchResult);
            return result;
        }
