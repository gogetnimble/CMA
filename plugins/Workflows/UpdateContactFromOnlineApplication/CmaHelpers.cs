...
namespace UpdateContactFromOnlineApplication
{
    ...
    {   
        ...
        public static EntityReference GetPhysicianSpecialty(IOrganizationService service, Guid contactId, ITracingService ts )
        {
            EntityReference result = null;

            string fetchXml = $@"<fetch no-lock='true' >
                                      <entity name='new_physicianspecialty' >
                                        <attribute name='new_specialtyid' />
                                        <filter>
                                          <condition attribute='new_contactid' operator='eq' value='{contactId}' />
            ...
            Entity physicianspecialty = CrmHelpers.GetFirstFromExecuteFetch(service, fetchXml);
            result = physicianspecialty?.GetAttributeValueEx<EntityReference>("new_specialtyid", ts, "physicianspecialty.");
            return result;
        }
